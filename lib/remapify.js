...
transformAliases = function transformAliases (b, results) {
  ...
  if (expandedAliasesCount) {
    ...
    b.on('reset', function onBundleReset () {
...
eachFile = function eachFile (b, file, pattern) {
  ...
  var cwd = pattern.cwd || processCwd
    ...
    , isIndexFile = new RegExp('index(?:' + b._extensions.join('|') + ')').test(splitPath[splitPath.length - 1])
    , expandedAliases = {}
  ...
  _.each(['/', '\\'], function forEachSep (sep) {
    var aliasPath = splitPath.join(sep)
      ...
      , aliasWithNoFilename = splitPath.slice(0, splitPath.length - 1).join(sep)
      , lastIndex
    ...
    // if the file name is index.[js|node], alias the folder as a module
    if (isIndexFile) {
      expandedAliases[aliasWithNoFilename] = aliasifyFilePath
    }
