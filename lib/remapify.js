...
module.exports = function(b, options){
  ...
  patterns.forEach(function(pattern){
    ...
    onGlobMatch = function onGlobMatch(file){
      var cwd = pattern.cwd || process.cwd()
      ...
      b.emit('remapify:file', absoluteFilePath, expandedAliases, null, pattern)
    }

    onGlobEnd = function onGlobEnd(err, matches){
      if (err) {
        log(err)
        return void b.emit('error', err)
      }

      matches.forEach(onGlobMatch)
      b.emit('remapify:files', matches, expandedAliases, null, pattern)
      done()
    }

    glob(pattern.src, pattern, onGlobEnd)
  })
}
