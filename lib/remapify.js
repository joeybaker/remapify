...
module.exports = function remapify(b, options){
  ...
  identity = function identity(alias){ return alias; }
    ...
    var g = new Glob(pattern.src, pattern)
      ...
      .on('match', function globMatch(file){
        ...
        _(['/', '\\']).forEach(function forEachSep(sep){
          ...
          if (aliasWithNoExt[1])
            var filteredAlias = filterFxn(aliasWithNoExt[1], path.dirname(alias), path.basename(aliasWithNoExt[1]));
            expandedAliases[filteredAlias] = aliasifyFilePath
        })
