...
module.exports = function(b, options){
  ...
  applyFilter = function(alias, filter){
      if (filter)
        // if the filter option is passed, apply it to the alias and return the result
        return filter(alias, path.dirname(alias), path.basename(alias))
      else
        // otherwise, return the un-filtered alias
        return alias
  }
  ...
  patterns.forEach(function(pattern){
    var g = new Glob(pattern.src, pattern)
      ...
      .on('match', function globMatch(file){
        ...
        expandedAliases[applyFilter(alias, pattern.filter)] = absoluteFilePath
        // if the file ext matches a known browserify extension, alias it, without the extensions
        if (aliasWithNoExt[1])
          expandedAliases[applyFilter(aliasWithNoExt[1], pattern.filter)] = absoluteFilePath
